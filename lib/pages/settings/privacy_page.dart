  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Privasi', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
            ...
            SettingsSection(
              ...
              children: [
                SettingsTile(
                  ...
                  trailing: Switch(
                    value: _readReceipt,
                    onChanged: (v) => setState(() => _readReceipt = v),
  ...
  Widget build(BuildContext context) {
    return SettingsTile(
      ...
      onTap: () => showModalBottomSheet(
        ...
        builder: (ctx) => _OptionSheet(label: label, options: options, selected: value, onSelect: (v) { onChanged(v); Navigator.pop(ctx); }),
      ),
    );
  }
}
...
class _OptionSheet extends StatelessWidget {
  ...
  const _OptionSheet({required this.label, required this.options, required this.selected, required this.onSelect});
                ...
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(color: opt == selected ? AppColors.primary : AppColors.divider, width: 2),
                    ),
                    child: opt == selected ? const Icon(Icons.check, color: Colors.white, size: 12) : null,
                  ),
                  const SizedBox(width: 12),
                  Text(opt, style: TextStyle(fontSize: 14, color: opt == selected ? AppColors.primary : AppColors.textPrimary, fontWeight: opt == selected ? FontWeight.w600 : FontWeight.normal)),
                ],
              ),
            ),
          )),
